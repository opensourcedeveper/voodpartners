		...
		elseif ($type == "report")
		{
			if ($this->session->userdata('logged_in')=='User')
			{
				if ($_POST)
				{
					...
					{
						$ReportDesc['meta_desc'] = "Tryangle Reports has added ".$_POST['title']." to its database of Market Research Reports. This report covers market size by types, applications and major regions. It also focuses on industry trends and developments, future market forecasts and key players of the market.";
					}
				...
				if ($_POST) {
					...
					if ($flag == 1) {
						for ($i=2; $i <= $arrayCount; $i++){
							...
							$slugdata=filter_var(trim(str_replace('_x000D_', '', $allDataInSheet[$i][$SheetDataKey['MetaKeyword']])), FILTER_SANITIZE_STRING);
							...
							if ($MetaDesctiption != ""){
								...
								$ReportDesc['meta_desc'] = "Tryangle Reports has added ".$Report['title']." to its database of Market Research Reports. This report covers market size by types, applications and major regions. It also focuses on industry trends and developments, future market forecasts and key players of the market.";
							}
							...
							$isExist = $this->data_model->get(array('title'=> $Report['title'], 'publisher_id'=> $Report['publisher_id']), NULL, array('id'), NULL, 'report');

							$isPriceSame = $this->data_model->get(array('id'=> $isExist[0]["id"], 'single_price'=> $ReportDesc['single_price'],'multi_price'=> $ReportDesc['multi_price'],'ent_price'=> $ReportDesc['ent_price']), NULL, array('id'), NULL, 'report_details');
							if ($isExist && $isPriceSame){
								$count = count($isExist);
							}
							else{
								$count = 0;
							...
							$existingData = $this->data_model->get(array('title'=> filter_var(trim(str_replace('_x000D_', '', $allDataInSheet[$i][$SheetDataKey['ReportTitle']])), FILTER_SANITIZE_STRING), 'publisher_id'=> $this->data_model->get(array('name' => $Publisher), Null, array('id'), NULL, 'publisher')[0]['id']), NULL, array('id'), NULL, 'report');


							if($existingData && $isPriceSame){
								$res = $this->data_model->edit($Report, array('title'=> $Report['title'],'publisher_id'=> $Report['publisher_id']),'report');
								$ReportDesc['id'] = $existingData[0]["id"];
								if ($res){
